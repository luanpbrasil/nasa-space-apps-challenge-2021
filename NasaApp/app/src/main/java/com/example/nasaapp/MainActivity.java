...
public class MainActivity extends AppCompatActivity implements Choreographer.FrameCallback {
    ...
    private double cameraDegreesPerSecond = 1;//0.41;
    protected boolean activityPaused;
    private RenderableLayer placemarksLayer;

    private int playPause = 1;

    public String urlRequest = "https://raw.githubusercontent.com/luamzi/nasa-space-apps-challenge-2021/data/data.json";
    ...
    public void doFrame(long frameTimeNanos) {
        ...
        try {
            ...
            for (int i = 0; i < keys.size(); i++) {
                ...
                double lat = placemarkHashMap.get(keys.get(i)).getPosition().latitude;
                double lon = placemarkHashMap.get(keys.get(i)).getPosition().longitude;
                double alt = placemarkHashMap.get(keys.get(i)).getPosition().altitude;
    ...
    public void startPlacemarks(){
        ...
        Placemark origin = new Placemark(Position.fromDegrees(0, 0, 7e5),
    ...
    private class NewsAsyncTask extends AsyncTask<String, Void, List<Placemark>> {
        @Override
        ...
        @Override
        protected void onPostExecute(List<Placemark> data) {
            super.onPostExecute(data);
            for(int i = 0; i < data.size(); i++){
                Placemark aux = data.get(i);
                Log.v("MainActivity.java", aux.getDisplayName());
                if(placemarkHashMap.containsKey(aux.getDisplayName())){
                    placemarkHashMap.get(aux.getDisplayName()).moveTo(wwd.getGlobe(), aux.getPosition());
                }
                else{
                    placemarkHashMap.put(aux.getDisplayName(), aux);
                    placemarksLayer.addRenderable(aux);
